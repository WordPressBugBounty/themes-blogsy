...
if ( ! class_exists( 'Blogsy_Customizer_Widget_Nav' ) ) :

	/**
	 * Blogsy Customizer widget class
	 */
	class Blogsy_Customizer_Widget_Nav extends Blogsy_Customizer_Widget {
		...
		public function __construct( array $args = [] ) {
			...
			$this->name        = __( 'Navigation', 'blogsy' );
			$this->description = __( 'Add a navigation menu.', 'blogsy' );
		...
		public function form(): void {
			...
			$has_menu_class = empty( $menus ) ? '' : 'blogsy-widget-nav-has-menu';
			?>
			<div class="blogsy-widget-nav-container <?php echo esc_attr( $has_menu_class ); ?>">
				<p class="blogsy-widget-nav-selected">
					<label for="widget-nav-<?php echo esc_attr( $this->id ); ?>-<?php echo esc_attr( $this->number ); ?>-menu">
						<?php echo esc_html_x( 'Select Menu:', 'Widget', 'blogsy' ); ?>
					</label>
					...
					<select id="widget-nav-<?php echo esc_attr( $this->id ); ?>-<?php echo esc_attr( $this->number ); ?>-menu" name="widget-nav-<?php echo esc_attr( $this->number ); ?>-menu" data-option-name="menu">
						<option value="0"><?php echo esc_html_x( '&mdash; Select &mdash;', 'Widget', 'blogsy' ); ?></option>
				...
				<p class="blogsy-widget-nav-empty">
					<?php
					printf(
						/* Translators: %1$s - anchor tag start. %2$s anchor tag end. */
						esc_html__( 'No menus found: %1$sCreate a new menu?%2$s', 'blogsy' ),
