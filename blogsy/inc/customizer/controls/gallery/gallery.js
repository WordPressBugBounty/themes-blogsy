...
( function ( api, $ ) {
	...
	api.ImageGalleryControl = api.Control.extend(
		{
			initialize: function ( id, options ) {
				...
				if ( ! args.params.type ) {
					args.params.type = 'blogsy-gallery';
				}
			...
			setAttachmentsData: function ( value ) {
				...
				_.each(
					...
					function ( id, index ) {
						...
						wp.media.attachment( id ).fetch().done(
							function () {
				...
				list.sortable(
					{
						...
						stop: function () {
							var selectedValues = [];
							list.find( '.image-gallery-thumbnail-wrapper' ).each(
								function () {
									var id;
	...
	api.controlConstructor['blogsy-gallery'] = api.ImageGalleryControl;
