...
function BlogsyLinkField($el) {
	...
	const setValue = (val,e) => {
		...
		var url = val ?.url ?.length > 25 ? val.url.substring( 0, 22 ) + '...' : val.url
		this.$el.find( '.link-url' ).attr( 'href', val.url ).html( url ).attr( 'title', val.url );
	...
	const open = () => {
		...
		var $textarea = jQuery( '<textarea id="blogsy-link-textarea" style="display:none;"></textarea>' );
		if ( ! jQuery( '#blogsy-link-textarea' ).length) {
			jQuery( 'body' ).append( $textarea );
		}

		var val = getNodeValue();
		wpLink.open( 'blogsy-link-textarea', val.url, val.title, null );
	}
	...
	const  onClose = () => {
		...
		jQuery( '#blogsy-link-textarea' ).remove();
	}
			...
			function ( j, v ) {
				...
				if ( 2 === param.length ) {
					...
					if ( coerce ) {
						...
						:
						'undefined' === val ? undefined // undefined
							:
...
( function ( api, $ ) {
	api.controlConstructor['blogsy-repeater'] = api.Control.extend(
		{
			ready: function () {
				var control = this;
				setTimeout(
			...
			_init: function () {
				...
				control.update_value = function () {
					...
					self.container.find( '[data-option]' ).each(
						function () {
				...
				control.autocorrect_range_input_number = function ( input_number, timeout ) {

					var range_input = input_number,
					range           = range_input.parent().find( 'input[type="range"]' ),
					value           = parseFloat( range_input.val() ),
					reset           = parseFloat( range.find( '.blogsy-reset-range' ).attr( 'data-reset_value' ) ),
				...
				if ( ! window._upload_blogsy ) {
					...
					window._upload_blogsy = wp.media(
						{
				...
				window._upload_blogsy.on(
					'close',
					function () {

						// get selections and save to hidden input plus other AJAX stuff etc.
						var selection = window._upload_blogsy.state().get( 'selection' );
				...
				window._upload_blogsy.on(
					...
					function () {
						...
						var media_attachment = window._upload_blogsy.state().get( 'selection' ).first().toJSON();
				...
				control.handleMedia = function ( $context ) {
					$( '.item-media', $context ).each(
						function () {
							var _item = $( this );
							// when remove item
							$( '.remove-button', _item ).on(
								'click',
								function ( e ) {
									e.preventDefault();
				...
				control.handleGradient = function ( $context ) {
					...
					range_input,
					value,
					blogsy_range_input_number_timeout,
					popup_content = control.container.find( '.popup-content' );

					// Range controls.

					$( '.item-gradient', $context ).each(
						function () {
							var _item = $( this );

							_item.find( '.blogsy-range-wrapper' ).each(
								function () {
									var $this = $( this );
									...
									$this.find( 'input.blogsy-range-input' ).on(
										'change keyup',
										function () {
											control.autocorrect_range_input_number( $( this ), 1000, blogsy_range_input_number_timeout );
										}
									).on(
										'focusout',
										function () {
											control.autocorrect_range_input_number( $( this ), 0, blogsy_range_input_number_timeout );
										}
									);
							...
							_item.on(
								...
								function () {
									...
									_item.find( '[data-dep-field="' + field + '"][data-dep-value="' + $( this ).val() + '"]' ).show();
								}
							);

							_item.find( '[data-option="background-type"], [data-option="gradient-type"]' ).trigger( 'change' );

							_item.on(
								'change',
								'.blogsy-select-wrapper',
								function () {
									control.update_value();
								}
							);
				...
				control.handleBackground = function ( $context ) {
					...
					$( '.item-design-options', $context ).each(
						function () {
							var _item = $( this );
									...
									var image = wp.media(
										...
										function () {
											...
											if ( '' !== imageUrl ) {
												...
												_item.find( '.advanced-settings' ).removeClass( 'hidden' ).addClass( 'up' );
											}
											...
											removeButton = _item.find( '.background-image-upload-remove-button' );
							...
							_item.on(
								...
								function ( e ) {
									...
									// Hide unnecessary controls.
									_item.find( '.background-image-advanced' ).hide();
									_item.find( '.advanced-settings' ).addClass( 'hidden' ).removeClass( 'up' );
							...
							// Reset default.
							_item.find( '.reset-defaults' ).on(
								'click',
								function () {

									var item, option_id;

									_item.find( '[data-option]' ).each(
										function () {
											item      = $( this );
				...
				control.linkPicker = function ( $context ) {
					if (item_link = $context.find( '.blogsy-field-link' )) {
						const a = new BlogsyLinkField( item_link );
						a.init();
					}
				}
				...
				control.actions = function ( $context ) {
					if ( control.params.live_title_id ) {
						...
						if ( 'blogsy_section_order_styling' === control.id ) {
							if ( 'click' !== $context.find( 'input.add_by' ).val() ) {
								format = '[live_title]';
							}
						}
						// Live title
						if ( control.params.live_title_id && 0 < $( '[data-live-id=\'' + control.params.live_title_id + '\']', $context ).length ) {
							var v = '';
							...
							} else if( $( '[data-live-id=\'' + control.params.live_title_id + '\']', $context ).is( '.input-url' ) ) {
								v = $( '[data-live-id=\'' + control.params.live_title_id + '\']', $context ).prev().val();
							} else {
				 ...
				 */
				control.repeaterTemplate = _.memoize(
					function () {
						var compiled,
				...
				control.intItem = function ( $context ) {
					...
					// Setup editor
					$( '.item-editor textarea', $context ).each(
						function () {
							control.editor( $( this ) );
						}
					);
...
jQuery( document ).ready(
	function ( $ ) {
		...
		icon_picker.find( '.c-icon-search input' ).attr( 'placeholder', Blogsy_Icon_Picker.search );
