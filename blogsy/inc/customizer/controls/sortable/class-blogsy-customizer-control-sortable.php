...
if ( ! class_exists( 'Blogsy_Customizer_Control_Sortable' ) ) :
	/**
	 * Blogsy Customizer custom select control class.
	 */
	class Blogsy_Customizer_Control_Sortable extends Blogsy_Customizer_Control {
		 ...
		 */
		public function __construct( $manager, $id, $args = [] ) {// phpcs:ignore Generic.CodeAnalysis.UselessOverridingMethod.Found
		...
		public function to_json() {
			...
			foreach ( $this->choices as $key => $value ) {
				if ( ! isset( $this->json['value'][ $key ] ) && isset( $this->json['default'][ $key ] ) ) {
		...
		protected function content_template() {
			?>
				<div class="blogsy-control-wrapper blogsy-sortable-wrapper">
					...
					<ul class="blogsy-sortable-control">
						<# for ( key in data.value ) { #>
							<li class="blogsy-sortable-item<# if ( _.has( data.value, key) && ! data.value[ key ] || ! _.has( data.value, key ) && _.has( data.default, key ) && ! data.default[ key ] ) { #> invisible<# } #>" data-value="{{ key }}">
								<# if ( data.sortable ) { #>
