<?php
/**
 * Blogsy Customizer Widget control class.
 *
 * @package     Blogsy
...
if ( ! class_exists( 'Blogsy_Customizer_Control_Widget' ) ) :
	/**
	 * Blogsy Customizer custom select control class.
	 */
	class Blogsy_Customizer_Control_Widget extends Blogsy_Customizer_Control {
		...
		public function to_json() {
			...
			if ( is_array( $this->widgets ) && ! empty( $this->widgets ) ) {
				...
				foreach ( $this->widgets as $widget_id => $args ) {
					...
					ob_start();
					?>
					<div data-widget-id="<?php echo esc_attr( $instance->id_base ); ?>" class="blogsy-widget-tpl-<?php echo esc_attr( $instance->id_base ); ?> blogsy-widget" data-max-uses=<?php echo esc_attr( $max_uses ); ?>>
		...
		protected function content_template() {
			?>
			<div class="blogsy-control-wrapper blogsy-widget-wrapper">

				<div class="blogsy-widget-container">

					<# _.each( data.value, function( template, i ) { #>
						{{{ template }}}
					<# }); #>

				</div><!-- END .blogsy-widget-container -->

				<div class="blogsy-add-widget-wrap">
					<button type="button" class="button blogsy-add-widget" data-location-title="{{{ data.label }}}" data-control="{{{ data.id }}}"><?php esc_html_e( 'Add Widget', 'blogsy' ); ?></button>
				</div>

				<div class="blogsy-widget-tmpls">
