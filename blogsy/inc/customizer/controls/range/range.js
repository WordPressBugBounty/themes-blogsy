;( function ( $ ) {
	...
	wp.customize.controlConstructor['blogsy-range'] = wp.customize.Control.extend(
		{
			...
			ready: function () {
				...
				control.container.find( '.blogsy-range-wrapper' ).rangeControl(
					{
			...
			save: function () {
				...
				if ( false === devices || devices === undefined || 0 == devices.length ) {
					value.value = parseFloat( this.container.find( '.blogsy-range-input' ).val() );
				} else {
					for ( var device in devices ) {
						value[ device ] = parseFloat( this.container.find( '.control-responsive.' + device + ' .blogsy-range-input' ).val() );
					}
				}

				if ( this.container.find( '.blogsy-control-unit' ).length ) {
					value.unit = this.container.find( '.blogsy-control-unit input:checked' ).val();
				}
	...
	( function ( $, undef ) {
		...
		RangeControl = {
			...
			_create: function () {
				...
				self.unit = el.find( '.blogsy-control-wrap' ).attr( 'data-unit' );
			...
			_addListeners: function () {
				...
				self.element.on(
					...
					function () {
						...
						range = $( this );
						input = range.parent().children( '.blogsy-range-input' );
				...
				self.element.on(
					...
					function () {
						...
						self.element.find( '.blogsy-reset-range' ).each(
							function ( index, el ) {
								...
								$el.closest( '.blogsy-range-wrapper' ).find( '.blogsy-control-unit input[type="radio"][value="' + $el.data( 'reset_unit' ) + '"]' ).click();
				...
				self.element.on(
					'click',
					'.blogsy-control-unit input',
					function () {

						var $unit = $( this );

						self.element.find( '.blogsy-control-wrap input[type="range"]' ).each(
							function ( index, el ) {
						...
						if ( 'px' === $unit.val() && ( 'rem' === self.unit || 'em' === self.unit ) ) {
							...
							self.element.find( 'input.blogsy-range-input' ).each(
								function ( index, el ) {
			...
			_initResponsiveControls: function () {
				...
				var self = this,
					el   = self.element,
					wrap = el.find( '.blogsy-control-wrap' );

				el.addClass( 'blogsy-control-responsive' );

				// Populate responsive switcher.
				if ( el.find( '.customize-control-title' ).length ) {

					var $switcher = $( '<ul class="blogsy-responsive-switchers"></ul>' );

					for ( var device in self.options.responsive ) {
						$switcher.append( '<li class="' + device + '"><span class="preview-' + device + '" data-device="' + device + '"><span class="blogsy-tooltip small-tooltip">' + device + '</span><i class="' + self.options.responsive[device].icon + '"></i></span></li>' );
					}
			...
			_initUnit: function () {
				...
				var template = wp.template( 'blogsy-control-unit' );
				...
				if ( 'object' === typeof self.options.unit ) {
					...
					$( '<span class="blogsy-range-unit">' + self.options.unit + '</span>' ).insertAfter( wrap.find( '.blogsy-range-input' ) );
				}

				wrap.parent().find( '.blogsy-control-unit input:checked' ).click();
			}
		};

		// Register the color picker as a widget.
		$.widget( 'blogsy.rangeControl', RangeControl );
	}( jQuery ) );
